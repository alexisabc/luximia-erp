            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700 md:divide-y-0">
                        {data.length > 0 ? (
                            /* ### CAMBIO 1: Capturamos el 'rowIndex' aquí ### */
                            data.map((row, rowIndex) => (
                                /* ### CAMBIO 2: Usamos 'rowIndex' para una key más fiable ### */
                                <tr key={row.id || rowIndex} className="block mb-4 p-4 rounded-lg bg-white shadow-md md:table-row md:mb-0 md:p-0 md:shadow-none md:rounded-none dark:bg-gray-800">
                                    {columns.map((col, colIndex) => (
                                        <td key={`${row.id || rowIndex}-${colIndex}`} className="block p-2 text-right border-b md:table-cell md:text-left md:p-4 md:border-b-0 dark:border-gray-700" data-label={col.header}>
                                            ...
                                            <div className="text-gray-900 dark:text-white mt-1 md:mt-0">
                                                {/* ### CAMBIO 3: Pasamos el 'rowIndex' a la función render ### */}
