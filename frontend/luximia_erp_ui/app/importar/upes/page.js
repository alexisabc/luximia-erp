...
import { importarUPEs } from '../../../services/api'; // Usamos la nueva función
import Link from 'next/link';

export default function ImportarUPEsPage() {
    // El código de React es casi idéntico al de Clientes, solo cambian los textos
    // y la lógica de la plantilla y la respuesta.
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await importarUPEs(formData); // <--- Llamamos a la nueva función
    ...
    // El resto de los manejadores (handleFileChange, etc.) son idénticos
    ...
    return (
        <div className="p-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-8">Importación Exclusiva de UPEs</h1>
            <div className="bg-white p-6 rounded-xl shadow-lg max-w-2xl">
                <div className="flex justify-between items-center mb-4">
                    <p className="text-gray-600">Sube un archivo <strong>.csv</strong> de UPEs.</p>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mt-6">
                        <button type="submit" disabled={!selectedFile || isUploading} className="w-full bg-blue-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-blue-700 disabled:bg-gray-400">
                            {isUploading ? 'Procesando...' : 'Subir UPEs'}
                        </button>
                    </div>
                </form>
                <div className="mt-6">
                    ...
                    {uploadResponse && (
                        <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded">
                            ...
                            <p className="text-sm mt-2">Nuevas: {uploadResponse.upes_creadas}, Actualizadas: {uploadResponse.upes_actualizadas}</p>
                        </div>
                    )}
                </div>
            </div>
