...
export default function ImportarPresupuestosPage() {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['cliente_id','upe_id','moneda_id','tipo_cambio_id','forma_pago_id','precio_m2','precio_lista','descuento','precio_con_descuento','enganche','saldo','plan_pago_id','fecha_entrega_pactada','negociaciones_especiales'];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await importarPresupuestos(formData);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-8">Importación de Presupuestos</h1>
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <p className="text-gray-600 dark:text-gray-300">Sube un archivo <strong>.csv</strong> con datos de presupuestos.</p>
          <button
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6">
            ...
            >
              {isUploading ? 'Procesando...' : 'Subir y Procesar Presupuestos'}
            </button>
          </div>
        </form>
