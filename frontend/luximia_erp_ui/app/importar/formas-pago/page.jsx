...
export default function ImportarFormasPagoPage() {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['enganche', 'mensualidades', 'meses', 'contra_entrega'];
    const csvContent = 'data:text/csv;charset=utf-8,' + headers.join(',');
    const link = document.createElement('a');
    link.setAttribute('href', encodeURI(csvContent));
    link.setAttribute('download', 'plantilla_formas_pago.csv');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await importarFormasPago(formData);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-8">Importación de Formas de Pago</h1>
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <p className="text-gray-600 dark:text-gray-300">Sube un archivo <strong>.csv</strong> con datos de formas de pago.</p>
          <button
