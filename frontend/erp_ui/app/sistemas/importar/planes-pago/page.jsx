...
export default function ImportarPlanesPagoPage() {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['cliente_id','upe_id','apartado_monto','moneda_apartado_id','fecha_apartado','forma_pago_enganche_id','monto_enganche','moneda_enganche_id','fecha_enganche','forma_pago_mensualidades_id','monto_mensualidades','moneda_mensualidades_id','forma_pago_meses_id','meses','monto_mensual','moneda_mensual_id','forma_pago_contra_entrega_id','monto_contra_entrega','moneda_contra_entrega_id'];
    const csvContent = 'data:text/csv;charset=utf-8,' + headers.join(',');
    const link = document.createElement('a');
    link.setAttribute('href', encodeURI(csvContent));
    link.setAttribute('download', 'plantilla_planes_pago.csv');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await importarPlanesPago(formData);
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-200 mb-8">Importación de Planes de Pago</h1>
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <p className="text-gray-600 dark:text-gray-300">Sube un archivo <strong>.csv</strong> con datos de planes de pago.</p>
          <button
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6">
            ...
            >
              {isUploading ? 'Procesando...' : 'Subir y Procesar Planes de Pago'}
            </button>
          </div>
        </form>
